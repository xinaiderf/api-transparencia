...
def overlay_videos_with_audio(video_base_path, video_overlay_path, output_path):
    ...
    video_combined = CompositeVideoClip([base_clip, overlay_resized.set_opacity(0.15)])
...
async def overlay_api(video_base: UploadFile = File(...), video_overlay: UploadFile = File(...)):
    ...
    try:
        overlay_videos_with_audio(temp_video_base, temp_video_overlay, temp_output_video)
